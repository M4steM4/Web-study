...
- template strings : 문자열 채워넣기(ex : ${firstname})
- eval() : 취약점때문에 사용 금지(js 구문 실행함수, 호이스팅시 콜스택 최상단에 오름 - 런타임 시간)
- 함수식 < 함수선언 : 콜스택에서 확인 용이, 함수이름 hoisting, ArrowFunction
- 고차 함수 -> 로직 정의된 외부함수로 이동 : Use arrow function
- 제너레이터 사용 자제, Don't transpile to ES5
- 무명함수, 명명함수는 컴파일 되기전에 변수가 hoist됨
- 지역변수는 함수내에서 전역변수보다 높은 우선순위를 가짐(콜스택에서 덮어씀, 함수선언이 빠르기 때문)
- setTimeout()안에서 함수는 전역범위에서 실행(window객체를 참조하기 때문)

- var를 사용 안할경우 함수참조보다 변수나 함수선언이 먼저
nodejs도 비동기, non-blocking 사용
